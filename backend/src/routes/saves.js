...
export async function toggleSave(request, env, videoId) {
  ...
  try {
    ...
    if (existing) {
      await env.DB.prepare(`DELETE FROM video_saves WHERE id = ?`)
    ...
    return error(err.message || "Failed to toggle save", 500, "SAVE_ERROR");
  }
}
...
export async function getSavedVideos(request, env) {
  ...
  try {
    ...
    return success({ videos: results });

  } catch (err) {
    return error(err.message || "Failed to load saved videos", 500, "SAVED_VIDEOS_ERROR");
  }
}